...
transformed data{
  int <lower=1> N_param;
  N_param = 7; // number of free parameters
  int idx;
}
...
//      test = 1-mu[(ses-1)*P+p , 1:t_p_s_p_ses[(ses-1)*P+p]];
  ...
  // 
  //     X_normal[,n] = 0.5 + 0.5 * erf((X[,n] - psyalpha) ./ (psybeta * sqrt(2)));
  //     
  //     mu[,n] = lapse + (1 - 2 * lapse) .* X_normal[,n];
  // 
  //   }
  // }
...
model{
  ...
  target += normal_lpdf(tau_b[2:7] | 0, 20)-normal_lccdf(0 | 0, 20);

  target += normal_lpdf(tau_w[1] | 0, 10)-normal_lccdf(0 | 0, 10);
  target += normal_lpdf(tau_w[2:7] | 0, 5)-normal_lccdf(0 | 0, 5);
