...
transformed parameters{
  ...
  psybeta[,2] = exp(param[,9]);
  
  lapse[,1] = inv_logit(param[,3]) / 2;
  lapse[,2] = inv_logit(param[,10]) / 2;
  
  intercept[,1] = param[,4];
  intercept[,2] = param[,11];
  
  beta[,1] = exp(param[,5]);
  beta[,2] = exp(param[,12]);
  
  sigma[,1] = exp(param[,6]);
  sigma[,2] = exp(param[,13]);
  
  ndt[,1] = inv_logit(param[,7]) .* min_RT[,1];
  ndt[,2] = inv_logit(param[,14])  .* min_RT[,2];
...
model{
  ...
  target += normal_lpdf(tau_u[8] | 0, 10)-normal_lccdf(0 | 0, 10);
  ...
  target += normal_lpdf(tau_u[11] | 0, 3)-normal_lccdf(0 | 0, 3);
