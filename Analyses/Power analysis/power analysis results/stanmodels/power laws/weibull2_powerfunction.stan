...
transformed parameters {
  ...
  alpha =  mu_intercept_alpha * (design_matrix[,1] ^ expo_alpha[1]) .* (design_matrix[,2] ^ expo_alpha[2]) .* (design_matrix[,3] ^ expo_alpha[3]);

  beta =  mu_intercept_beta * (design_matrix[,1] ^ expo_beta[1]) .* (design_matrix[,2] ^ expo_beta[2]) .* (design_matrix[,3] ^ expo_beta[3]);
  
  X_norm = 1-exp(-(x ./ alpha)^(1/beta));
...
model {
  ...
  target += normal_lpdf(expo_alpha | 0, 3);
  target += normal_lpdf(expo_beta | 0, 3);
