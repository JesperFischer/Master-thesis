...
get_ttest = function(parameters){
  ...
  effectsize = (mean(sessions2)-mean(sessions1) / (sqrt(sd(sessions1)^2 + sd(sessions2)^2) - 2 * 0 * sd(sessions1) * sd(sessions2))) * sqrt(2 * (1-cor))
...
q %>% group_by(sd_dif, cor) %>% dplyr::summarize(mean = mean(sd_ses2), correlation = mean(cor)) %>% mutate(calculated = sqrt(1^2+sd_dif^2))
