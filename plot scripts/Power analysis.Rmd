...
mod = cmdstanr::cmdstan_model(here::here("realshit","Power analysis","pathfinder","reaction time","power_analysis_psi","stanmodels","power laws","logistic_power_int.stan"),stanc_options = list("O1"))
...
marginals_histogram = marginals %>% select(c("expo_alpha1","expo_alpha2","expo_alpha3","intercept_alpha",
...
ggsave(here::here("Figures","marginals_histogram.PNG"), marginals_histogram, height = 6,width = 7.2)


```


#  plot 20 observed effectsize distribution
...
# plot 21 probability of rejecting effectsize on probability of effectsize:
...
# plot 22 with and without sampling varability  and Gpower
...
subtract = 0.015
linedata = df_un %>% group_by(trials, subs) %>% summarize(mean_power = round(mean(power),3)) %>% filter(mean_power < 0.8+subtract & mean_power > 0.8-subtract) %>% filter(trials < 199)
...
pun = df_un %>% 
  ...
  scale_y_continuous("Subjects", breaks = seq(10,55,by = 10))+
...
#  geom_line(data = linedata, aes(x = trials, y = subs), col = "red")+
  scale_fill_continuous(type = "viridis", breaks = c(0,0.2,0.4,0.6,0.8,1))+theme_classic()+
  scale_y_continuous("Subjects", breaks = seq(10,50,by = 10), limits = c(10,50))+
  scale_x_continuous("Trials", breaks = seq(10,200,by = 30), limits = c(10,200))
...
power_sampling_varability_and_gpower = p_noun+geom_segment(aes(y = 20, yend = 20, x = 10, xend = 200),linetype = 2, col = "red")+
